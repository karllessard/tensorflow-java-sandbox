    ...
    try (EagerSession session = EagerSession.create()) {
      Operation asrt =
          opBuilder(session, "Assert", "assert")
              .addInput(TestUtil.constant(session, "Cond", Tensors.scalar(true)))
              .addInputList(new Output<?>[] {
                  TestUtil.constant(session, "Error", Tensors.scalar(-1))
              })
              .build();
      try {
  ...
  public void setDevice() {
    try (EagerSession session = EagerSession.create()) {
      opBuilder(session, "Add", "SetDevice")
          ...
          .addInput(TestUtil.constant(session, "Const1", Tensors.scalar(2)))
          .addInput(TestUtil.constant(session, "Const2", Tensors.scalar(4)))
          .build();
    }
  }
  ...
  public void setAttrs() {
    ...
    try (EagerSession session = EagerSession.create()) {
      ...
      opBuilder(session, "RandomUniform", "DataTypeAndInt")
          .addInput(TestUtil.constant(session, "RandomUniformShape", Tensors.vector(new int[]{1})))
          .setAttr("seed", 10)
          .setAttr("dtype", TFloat.DTYPE)
          .build();
      // list(int), string
      opBuilder(session, "MaxPool", "IntListAndString")
          .addInput(TestUtil.constant(session, "MaxPoolInput", Tensors.of(TFloat.DTYPE, Shape.make(2, 2, 2, 2))))
      ...
      opBuilder(session, "FractionalMaxPool", "FloatList")
          .addInput(TestUtil.constant(session, "FractionalMaxPoolInput", Tensors.of(TFloat.DTYPE, Shape.make(2, 2, 2, 2))))
          .setAttr("pooling_ratio", new float[] {1.0f, 1.44f, 1.73f, 1.0f})
          .build();
      // shape
      opBuilder(session, "EnsureShape", "ShapeAttr")
          .addInput(TestUtil.constant(session, "Const", Tensors.of(TFloat.DTYPE, Shape.make(2, 2))))
      ...
      opBuilder(session, "FIFOQueue", "queue")
          ...
          .build();
      // bool
      opBuilder(session, "All", "TBool")
          .addInput(TestUtil.constant(session, "Const", Tensors.vector(new boolean[] {true, true, false})))
          .addInput(TestUtil.constant(session, "Axis", Tensors.scalar(0)))
          .setAttr("keep_dims", false)
          .build();
      // float
      opBuilder(session, "ApproximateEqual", "Float")
          .addInput(TestUtil.constant(session, "Const1", Tensors.scalar(10.00001f)))
          .addInput(TestUtil.constant(session, "Const2", Tensors.scalar(10.00000f)))
