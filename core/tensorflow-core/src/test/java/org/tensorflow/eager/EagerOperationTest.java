...
import org.tensorflow.eager.EagerOperation;
import org.tensorflow.eager.EagerOperationBuilder;
...
public class EagerOperationTest {
  ...
  public void outputDataTypeAndShape() {
    try (EagerSession session = EagerSession.create();
        Tensor<Int32> t = Tensors.create(new int[2][3])) {
      EagerOperation op =
          opBuilder(session, "Const", "OutputAttrs")
              .setAttr("dtype", Int32.TYPE)
              .setAttr("value", t)
              .build();
      assertSame(Int32.TYPE, op.dtype(0));
  ...
  public void outputTensor() {
    try (EagerSession session = EagerSession.create()) {
      ...
      assertSame(Int32.TYPE, add.dtype(0));
    }
  }
