...
public class GraphOperationBuilderTest {
  ...
  public void failWhenMixingOperationsOnDifferentGraphs() {
    try (Graph g1 = new Graph();
        Graph g2 = new Graph()) {
      Output<TInt32> c1 = TestUtil.constant(g1, "C1", Tensors.scalar(3));
      Output<TInt32> c2 = TestUtil.constant(g2, "C2", Tensors.scalar(3));
  ...
  public void setAttr() {
    ...
    try (Graph g = new Graph()) {
      ...
      g.opBuilder("RandomUniform", "Int")
          .addInput(TestUtil.constant(g, "RandomUniformShape", Tensors.vector(new int[] {1})))
          .setAttr("seed", 10)
      ...
      g.opBuilder("MaxPool", "IntList")
          .addInput(TestUtil.constant(g, "MaxPoolInput", Tensors.of(TFloat.DTYPE, Shape.make(2, 2, 2, 2))))
      ...
      g.opBuilder("FractionalMaxPool", "FloatList")
          .addInput(TestUtil.constant(g, "FractionalMaxPoolInput", Tensors.of(TFloat.DTYPE, Shape.make(2, 2, 2, 2))))
    ...
    try (Graph g = new Graph()) {
      Output<?> n =
          g.opBuilder("Placeholder", "unknown")
              .setAttr("dtype", TFloat.DTYPE)
              .setAttr("shape", Shape.make(Shape.UNKNOWN_SIZE))
              .build()
              .output(0);
      assertEquals(-1, n.shape().numDimensions());
      assertSame(TFloat.DTYPE, n.dataType());

      n =
          g.opBuilder("Placeholder", "batch_of_vectors")
              .setAttr("dtype", TFloat.DTYPE)
              .setAttr("shape", Shape.make(Shape.UNKNOWN_SIZE, 784))
              .build()
              .output(0);
  ...
  public void addControlInput() {
    try (Graph g = new Graph();
        ...
        TBool yes = Tensors.scalar(true);
        TBool no = Tensors.scalar(false)) {
  ...
  private static void testSetAttrShapeList(Shape[] shapes) {
    try (Graph g = new Graph();
      ...
      NdArray<Integer> matrix = NdArrays.of(Integer.class, Shape.make(2, 2));
      Output<TInt32> c1 = TestUtil.constant(g, "const1", Tensors.copyOf(TInt32.DTYPE, matrix));
      NdArray<Integer> matrix3d = NdArrays.of(Integer.class, Shape.make(2, 2, 2));
      Output<TInt32> c2 = TestUtil.constant(g, "const2", Tensors.copyOf(TInt32.DTYPE, matrix3d));
      Operation enqueue =
