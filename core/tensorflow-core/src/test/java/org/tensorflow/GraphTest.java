...
public class GraphTest {
  ...
  public void graphDefRoundTrip() {
    ...
    try (Graph g = new Graph(); TInt32 a = Tensors.of(TInt32.DTYPE, Shape.make(2, 2))) {
  ...
  public void iterateOverOperations() {
    try (Graph g = new Graph()) {
      ...
      operations.add(TestUtil.constant(g, "Const-A", Tensors.scalar(1.0f)).op());
      operations.add(TestUtil.constant(g, "Const-B", Tensors.scalar(23)).op());
      operations.add(TestUtil.constant(g, "Const-C", Tensors.scalar(1.618)).op());
  ...
  public void addGradientsToGraph() {
    try (Graph g = new Graph();
      ...
      assertSame(TFloat.DTYPE, grads0[0].dataType());

      Output<?>[] grads1 = g.addGradients(y2, toArray(x1, x2));
      assertNotNull(grads1);
      assertEquals(2, grads1.length);
      assertSame(TFloat.DTYPE, grads1[0].dataType());
      assertSame(TFloat.DTYPE, grads1[1].dataType());
      
      try (TFloat c1 = Tensors.scalar(3.0f);
          TFloat c2 = Tensors.scalar(2.0f);
        ...
        assertEquals(108.0f, (Float)outputs.get(0).get(), 0.0f);
        assertEquals(6.0f, (Float)outputs.get(1).get(), 0.0f);
        assertEquals(1.0f, (Float)outputs.get(2).get(), 0.0f);
      }
    }
  }
  ...
  public void addGradientSumsToGraph() {
    try (Graph g = new Graph();
      ...
      assertSame(TFloat.DTYPE, grad[0].dataType());

      try (TFloat c = Tensors.scalar(3.0f);
        ...
        assertEquals(114.0f, (Float)output.get(), 0.0f);
      }
    }
  }
  ...
  public void addGradientsWithInitialValuesToGraph() {
    try (Graph g = new Graph();
      ...
      try (TFloat c = Tensors.scalar(3.0f);
        ...
        assertEquals(108.0f, (Float)output.get(), 0.0f);
      }
    }
  }
      ...
      Graph.WhileSubgraphBuilder condGraphBuilder =
          new Graph.WhileSubgraphBuilder() {
            ...
            public void buildSubgraph(
              ...
              Output<TInt32> sixteen = TestUtil.constant(condGraph, "sixteen", Tensors.scalar(16));
              // condInputs[0] < 16
              Output<?> condOutput =
                  condGraph
                      .opBuilder("Less", "cond")
  ...
  public void buildWhileLoopMultipleInputs() {
    try (Graph g = new Graph();
      ...
      try (TInt32 c1 = Tensors.scalar(2);
          TInt32 c2 = Tensors.scalar(5);
        ...
        assertEquals(16, outputs.get(0).get()); // ((2^2)^2)
        assertEquals(625, outputs.get(1).get()); // ((5^2)^2)
      }
    }
  }
