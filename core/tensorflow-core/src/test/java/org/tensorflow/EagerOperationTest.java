...
public class EagerOperationTest {
  ...
  public void outputDataTypeAndShape() {
    try (EagerSession session = EagerSession.create();
        TInt32 t = Tensors.of(TInt32.DTYPE, Shape.make(2, 3))) {
      EagerOperation op =
          opBuilder(session, "Const", "OutputAttrs")
              .setAttr("dtype", t.dataType())
              .setAttr("value", t)
              .build();
      assertSame(TInt32.DTYPE, op.dtype(0));
      assertEquals(2, op.shape(0).numElements(0));
      assertEquals(3, op.shape(0).numElements(1));
    }
  }
    ...
    try (EagerSession session = EagerSession.create()) {
      EagerOperation add =
          opBuilder(session, "Add", "CompareResult")
              ...
              .build();
      assertEquals(6, add.tensor(0).get());

      // Validate that we retrieve the right shape and datatype from the tensor
      // that has been resolved
      assertEquals(0, add.shape(0).size());
      assertSame(TInt32.DTYPE, add.dtype(0));
    }
  }
  ...
  public void inputAndOutputListLengths() {
    try (EagerSession session = EagerSession.create()) {
      Output<TFloat> c1 = TestUtil.constant(session, "Const1", Tensors.vector(new float[] {1f, 2f}));
      Output<TFloat> c2 = TestUtil.constant(session, "Const2", Tensors.vector(new float[] {3f, 4f}));
    ...
    try (EagerSession session = EagerSession.create()) {
      EagerOperation op =
          opBuilder(session, "UniqueWithCountsV2", "unq")
              .addInput(TestUtil.constant(session, "Const1", Tensors.vector(new int[] {1, 2, 1})))
              .addInput(TestUtil.constant(session, "Axis", Tensors.vector(new int[] {0})))
