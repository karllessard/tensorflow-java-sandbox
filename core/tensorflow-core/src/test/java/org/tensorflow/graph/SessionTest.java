...
public class SessionTest {
  ...
  public void runUsingOperationNames() {
    try (Graph g = new Graph();
      ...
      try (Tensor<Int32> x = Tensors.create(new int[][] {{5}, {7}});
  ...
  public void runUsingOperationHandles() {
    try (Graph g = new Graph();
        Session s = new Session(g)) {
      TestUtil.transpose_A_times_X(g, new int[][] {{2}, {3}});
      Output<Int32> feed = g.operation("X").output(0);
      Output<Int32> fetch = g.operation("Y").output(0);
      try (Tensor<Int32> x = Tensors.create(new int[][] {{5}, {7}});
          TestUtil.AutoCloseableList<Tensor<?>> outputs =
  ...
  public void runUsingColonSeparatedNames() {
    try (Graph g = new Graph();
      ...
      try (Tensor<Int32> fetched =
          s.runner().fetch("Split:1").run().get(0).expect(Int32.class)) {
        final int[] expected = {3, 4};
        assertArrayEquals(expected, fetched.copyTo(new int[2]));
      }
      // Feed using colon separated names.
      try (Tensor<Int32> fed = Tensors.create(new int[] {4, 3, 2, 1});
          Tensor<Int32> fetched =
              s.runner()
                  .feed("Split:0", fed)
                  .feed("Split:1", fed)
                  .fetch("Add")
