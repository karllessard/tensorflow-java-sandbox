...
/** Unit tests for {@link GraphOperation}. */
...
public class GraphOperationTest {
  ...
  public void outputListLengthFailsOnInvalidName() {
    try (Graph g = new Graph()) {
      Operation op =
          g.opBuilder("Add", "Add")
              .addInput(TestUtil.constant(g, "x", Tensors.scalar(1)))
              .addInput(TestUtil.constant(g, "y", Tensors.scalar(2)))
              .build();
  ...
  public void operationEquality() {
    ...
    try (Graph g = new Graph()) {
      Operation newOp1 = TestUtil.constant(g, "op1", Tensors.scalar(1)).op();
  ...
  public void operationToString() {
    try (Graph g = new Graph()) {
      Operation op = TestUtil.constant(g, "c", Tensors.vector(new int[]{1})).op();
  ...
  public void outputEquality() {
    try (Graph g = new Graph()) {
      Output<TInt32> output = TestUtil.constant(g, "c", Tensors.scalar(1));
  ...
  public void outputToString() {
    try (Graph g = new Graph()) {
      Output<TInt32> output = TestUtil.constant(g, "c", Tensors.vector(new int[]{1}));
  ...
  private static int split(int[] values, int num_split) {
    try (Graph g = new Graph()) {
      return g.opBuilder("Split", "Split")
          ...
          .addInput(TestUtil.constant(g, "values", Tensors.vector(values)))
