...
class EagerOperation extends AbstractOperation {
  ...
  @Override
  protected long getUnsafeNativeHandle(int outputIndex) {
    return nativeRef.outputHandles[outputIndex];
  }

  @Override
  protected Shape shape(int outputIndex) {
    ...
    long[] dimensionSizes = new long[numDims(outputNativeHandle)];
    for (int i = 0; i < dimensionSizes.length; ++i) {
      dimensionSizes[i] = dim(outputNativeHandle, i);
    }
    return Shape.make(dimensionSizes);
  }

  @Override
  protected DataType<?> dtype(int outputIndex) {
    ...
    long outputNativeHandle = getUnsafeNativeHandle(outputIndex);
    return DataTypes.fromOrdinal(dataType(outputNativeHandle));
  }

  @Override
  protected Tensor<?> tensor(int outputIndex) {
    Tensor<?> tensor = outputTensors.get(outputIndex);
    if (tensor == null) {
