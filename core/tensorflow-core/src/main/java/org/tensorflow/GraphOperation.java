...
public final class GraphOperation extends AbstractOperation {
  ...
  @Override
  protected long getUnsafeNativeHandle(int outputIdx) {
    return getUnsafeNativeHandle();
  }

  @Override
  protected Shape shape(int outputIdx) {
    Graph.Reference r = graph.ref();
    try {
      return Shape.make(shape(r.nativeHandle(), getUnsafeNativeHandle(), outputIdx));
    } finally {
      r.close();
    }
  }
  ...
  @Override
  protected DataType<?> dtype(int outputIdx) {
    Graph.Reference r = graph.ref();
    try {
      return DataTypes.fromOrdinal(dtype(r.nativeHandle(), getUnsafeNativeHandle(), outputIdx));
    } finally {
      r.close();
    }
  }
  ...
  @Override
  protected Tensor<?> tensor(int outputIdx) {
